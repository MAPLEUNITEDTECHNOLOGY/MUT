...
interface CloudTrustStatsProps {
  stats: Stat[];
}
...
function AnimatedCounter({ value, suffix, label }: Stat) {
      ...
      ([entry]) => {
        if (entry.isIntersecting && !hasAnimated.current) {
          ...
          const animate = (now: number) => {
            ...
            setCount(isDecimal ? parseFloat(current.toFixed(decimals)) : Math.floor(current));
  ...
  }, [numericValue, isDecimal, decimals]);

  const displayValue = isDecimal ? count.toFixed(decimals) : count;
...
export default function CloudTrustStats({ stats }: CloudTrustStatsProps) {
  return (
    <motion.section 
      ...
      className="relative w-full bg-[#0B0F19] border-b border-white/5 z-20 -mt-10 md:-mt-20 rounded-t-[40px] md:rounded-t-[60px] shadow-[0_-20px_40px_rgba(0,0,0,0.3)]"
    >
