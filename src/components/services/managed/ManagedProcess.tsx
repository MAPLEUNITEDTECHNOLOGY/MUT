...
interface ManagedProcessProps { sectionLabel: string; headline: string; steps: ProcessStep[]; }

export default function ManagedProcess({ sectionLabel, headline, steps }: ManagedProcessProps) {
  return (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 relative">
          ...
          {steps.map((step, idx) => (
            <motion.div key={idx} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: idx * 0.15 }} className="flex flex-col items-center text-center group relative z-10">
              <div className="w-18 h-18 rounded-full bg-[#faf8ff] border-2 border-slate-200 flex items-center justify-center text-slate-400 font-black text-xl group-hover:border-[#e01030] group-hover:text-[#e01030] group-hover:shadow-[0_0_20px_rgba(224,16,48,0.15)] transition-all duration-300 mb-8 w-[72px] h-[72px]">
                {step.number}
              </div>
