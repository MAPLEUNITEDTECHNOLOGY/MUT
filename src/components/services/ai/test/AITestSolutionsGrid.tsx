    ...
    <section className="py-32 bg-[#f8fafc] relative overflow-hidden">
      <div className="container mx-auto px-6 md:px-12 lg:px-24">
        <div className="flex flex-col items-start mb-20">
          <span className="text-[11px] font-black uppercase tracking-[0.4em] text-red-700 mb-6 block">
          ...
          <h2 className="text-[#020617] text-4xl md:text-6xl font-black tracking-tighter mb-8 max-w-3xl leading-[0.9]">
            ...
            <span className="text-red-700 underline decoration-cyan-400/30">intelligence layers.</span>
          </h2>
        </div>
...
function BentoCard({ item, className }: { item: any, className?: string }) {
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className={`glass-card-light rounded-3xl p-8 flex flex-col relative overflow-hidden group cursor-pointer hover:border-red-500/40 transition-colors duration-500 ${className}`}
    >
      <div className="relative z-10 h-full flex flex-col">
        <div className="flex items-start justify-between mb-8">
          ...
          <span className="text-[10px] font-black text-red-700 uppercase tracking-widest bg-cyan-50 px-3 py-1.5 rounded-full">
            {item.roi}
          </span>
        </div>
      ...
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(224,16,48,0.05),transparent_70%)] opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
    </motion.div>
  );
}
